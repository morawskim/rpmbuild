...
Summary:        php53 Core Files
License:        PHP-3.01
Requires:       timezone

%description
This package contains the php53 core files, including PHP binary (CLI)
...
export PHPIMAP_SHARED_LIBADD='-lssl'
...
%{__sed} -i 's=@@VERSION@@=%{version}=g; s=@@PHP_PREFIX@@=%{base_dir}=g; s=notify=simple=' %{buildroot}%{_unitdir}/%{name}-fpm.service
...
-  add systemd service for php-fpm
